...
### Wan2.2 I2V Quantization (SVDQuant W4A4)

Wan2.2 I2V (Image-to-Video) uses a **two-stage denoising architecture** (similar to MoE): a **high-noise model** handles early denoising steps and a **low-noise model** handles final denoising steps. Both models need to be calibrated and quantized **separately**.

The two stages are based on [Wan2.2-Lightning](https://huggingface.co/lightx2v/Wan2.2-Lightning) 4-step LoRA adapters (fused into the base [Wan2.2-I2V-A14B](https://huggingface.co/Wan-AI/Wan2.2-I2V-A14B-Diffusers) transformer).

#### Step 0: Calibration Dataset Preparation

The calibration dataset (`examples/diffusion/prompts/wan_i2v.yaml`) contains 128 diverse video prompts paired with reference images. **The first-frame images are generated by a separate diffusion model (FLUX.1-schnell)**, not from real video frames.

The text prompts for image generation and video generation are defined in `video_prompts.json`. Use the provided script to generate the first-frame images:

```bash
python generate_i2v_dataset.py
```

This will use FLUX.1-schnell to generate 128 images (832x480) from the `image_prompt` field in `video_prompts.json` and save them to the `images/` directory.

#### Step 1: Collect Calibration Data

Collect calibration data for **both** the high-noise and low-noise models (parameters are pre-configured):

```bash
# High-noise model
python -m deepcompressor.app.diffusion.dataset.collect.calib_wan

# Low-noise model (modify the config path in calib_wan.py to use wan2-i2v-low-noise.yaml)
python -m deepcompressor.app.diffusion.dataset.collect.calib_wan
```

#### Step 2: Model Quantization

Quantize each model separately (parameters are pre-configured):

```bash
# High-noise model
python -m deepcompressor.app.diffusion.ptq_wan

# Low-noise model (modify the config path in ptq_wan.py to use wan2-i2v-low-noise.yaml)
python -m deepcompressor.app.diffusion.ptq_wan
```

#### Step 3: Convert to Nunchaku Format

Convert the quantized checkpoints to [Nunchaku](https://github.com/mit-han-lab/nunchaku) format for deployment:
